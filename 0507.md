...
```SQL
SELECT GREATEST(3000,1500,2100,5000),LEAST(3000,1500,2100,5000)FROM DUAL;SELECT DEPTNO,ENAME,SAL,DECODE(GREATEST(SAL,5000),SAL,'HIGH',DECODE(GREATEST(SAL,2500),SAL,'MID','LOW'))FROM EMPORDER BY DEPTNO;
```
...
```SQL
SELECT DEPTNO, ENAME, SAL,COMM,// NULL ??DECODE(GREATEST(COMM,5000),COMM,'HIGH',DECODE(GREATEST(COMM,2500),COMM,'MID','LOW'))FROM EMPORDER BY DEPTNO;
```
...
CASE WHEN COMM>= 5000 THEN 'HIGH'
	 ...
	 WHEN COMM BETWEEN300 AND2500 THEN 'LOW'
ELSE
	'UNKNOWN'
END
FROM EMPORDER BY DEPTNO;
...
```sql
SELECT SAL, CASE    WHEN SAL >= 1000 THEN 1 WHEN SAL >= 2000 THEN 2WHEN SAL >= 3000 THEN 3WHEN SAL >= 4000 THEN 4WHEN SAL >= 5000 THEN 5ELSE0ENDAS SAL_CHKFROM EMPORDER BY SAL;
```

=> 첫 번째 when에서 다 걸려버리는데?
...
=> WHERE절로 사용하면 RVALUE가 LVALUE의 형으로 바뀌지만, LIKE 연산자의 경우 LIKE 다음에는 문자형이 와야하기 때문에 SAL이 숫자에서 문자로 바꾸어 LIKE 연산자를 수행한다. 
